(define	(domain	uberPool)
			...
			(:types		location locatable time - object
						pump - location
						person uber - locatable
						driver customer - person)

			(:predicates		(at ?locata - locatable ?loc - location)
			...
			(:functions		(distance ?loc1 ?loc2 - location)
						(time-surge)
						(time)
						(fuel-level ?ube - uber)
			...
			(:durative-action	DRIVE
				...
				:condition	(and	(overall	(on-shift ?driv))
							(overall	(in-driv ?driv ?ube))
							(overall	(> (fuel-level) 1))
							(at start	(at ?ube ?loc-from)))
				:effect		(and	(at end	(at ?ube ?loc-to))
							(at end	(not	(at ?ube ?loc-from))
							(decrease (fuel-level ?ube) (* #t (* (time-surge) (* (speed ?ube) (0.1)))))))
)
